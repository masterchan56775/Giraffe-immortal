...
class TestTokenTracker:
    ...
    def test_record_returns_cost(self):
        cost = self.tracker.record("gemini-3-flash-preview", 1000, 500)
        assert cost > 0

    def test_daily_cost_accumulates(self):
        self.tracker.record("gemini-3-flash-preview", 1000, 500)
        self.tracker.record("gemini-3-flash-preview", 2000, 1000)
        daily = self.tracker.daily_cost()
        assert daily > 0

    def test_by_model_tracking(self):
        self.tracker.record("gemini-3-flash-preview", 500, 200, "session_1")
        self.tracker.record("claude-haiku-4.5", 300, 100, "session_1")
        by_model = self.tracker.by_model()
        assert "gemini-3-flash-preview" in by_model
        assert "claude-haiku-4.5" in by_model

    def test_total_stats_structure(self):
        self.tracker.record("gemini-3-flash-preview", 100, 50)
